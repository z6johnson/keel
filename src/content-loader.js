...
export async function loadBeatContent(beat) {
    // Inline content
    if (beat.content) {
...
async function loadFromSource(source, _role) {
...
async function loadApiContent(url, transform) {
...
function getApiKey() {
    ...
    return params.get('fogbell_key') ?? import.meta.env?.VITE_FOGBELL_KEY ?? undefined;
}
function normalizeFogBellData(data) {
...
export async function preloadBeats(beats, startIndex, cache, count = 3) {
    const targets = [];
