...
async function boot() {
    ...
    // Restore color mode
    const savedMode = sessionStorage.getItem('keel-color-mode');
    const initialMode = savedMode ?? 'dark';
    document.documentElement.setAttribute('data-mode', initialMode);
    // Load manifest
    const manifest = await loadManifest('/content/manifest.json');
    ...
    const colorMode = (document.documentElement.getAttribute('data-mode') ?? 'dark');
