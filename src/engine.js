...
export function buildFlatBeats(manifest, selectedModuleIds) {
...
export function createEngine(manifest, flatBeats, initialColorMode = 'dark') {
    const listeners = [];
    const state = {
        flatBeats,
        currentIndex: 0,
    ...
    function getBeatInfo() {
        ...
        const moduleBeats = state.flatBeats.filter(b => b.moduleId === beat.moduleId && !b.id.startsWith('__transition-'));
